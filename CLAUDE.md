...
Built and verified end-to-end: auth foundation (validation, Swagger, Redis), core CV flow (templates, CV/section/entry CRUD, dashboard/editor persistence), upload + AI parsing (`POST /uploads` — PDF sent natively to Gemini as multimodal input, DOCX text-extracted via `mammoth` first; `POST /cvs/from-upload` atomically creates the CV from the reviewed result), and "Improve with AI" suggestions (`POST /cvs/:cvId/ai-suggestions/generate` scoped to WHOLE_CV/SUMMARY/EXPERIENCE, `PATCH .../ai-suggestions/:id` for accept/reject/edit/undo, `POST .../ai-suggestions/apply` to atomically write ACCEPTED/EDITED text into the target entries). `AiSuggestion.entryId`/`fieldKey`/`label` were added to the schema for this. Parsing/generation are synchronous (no job queue yet) — acceptable for now, revisit if latency becomes a problem.

Not built yet: JD tailoring/match-scoring (will reuse `GeminiService` and most of the suggestion-apply mechanics above), and PDF/DOCX/link export (no PDF rendering approach chosen yet).

**Gemini cost caution**: `GEMINI_API_KEY` is a real, billed key. Don't call an endpoint that hits Gemini (`POST /uploads`, `POST /cvs/:cvId/ai-suggestions/generate`) without the user's awareness — check with them before firing real requests during a session, even for verification.

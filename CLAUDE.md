...
Also built: PDF/DOCX/link export (`POST /cvs/:cvId/exports` with `format: PDF|DOCX|LINK`). PDF renders the CV to HTML (single/two-column per the template's `structureJson.layout`, styled by `styleOverridesJson`) via a headless Chromium instance (`common/pdf/PdfService`, one Puppeteer browser kept alive for the app's lifetime — launching Chromium per-request would be too slow); DOCX builds an equivalent linear Word doc via the `docx` package. Both are written to disk under `EXPORT_DIR` and served back through an authenticated, ownership-checked `GET /cvs/:cvId/exports/:exportId/download` (`StreamableFile`, not through the `TransformInterceptor` envelope). LINK creates (or reuses, idempotently) a durable `shareSlug`; `GET /public/cvs/:shareSlug` (no auth) returns a *live*, hand-picked-safe projection of the CV — title/contact/style/template/sections only, deliberately excluding `userId`/`cvId`/`parentCvId`/`sourceUploadId`/timestamps, since anyone with the link can hit it. Puppeteer bundles headless Chromium (~650MB) — a real, deliberate infra cost; if this ever gets containerized, the image needs Chromium-compatible system libs.

Not built yet: JD tailoring's URL/file ingestion, and export's "Include cover letter" toggle from the mockup (no cover-letter model exists — out of scope, same as the original plan).
